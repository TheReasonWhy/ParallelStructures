#ifndef LOCK_FREE_QUEUE_PACKAGED_TASK_H
#define LOCK_FREE_QUEUE_PACKAGED_TASK_H
...
template<typename T>
class lock_free_queue_packaged_task //: public Parallel_container<T, lock_free_queue>
{
...
public:
    lock_free_queue_packaged_task():head(new node),tail(head.load()){}//
    lock_free_queue_packaged_task(const lock_free_queue_packaged_task &other)=delete;
    lock_free_queue_packaged_task& operator=(const lock_free_queue_packaged_task &other)=delete;

    ~lock_free_queue_packaged_task(){
        while(pop());
    }
    ...
    void set_new_tail(counted_node_ptr &old_tail,
        ...
        while(!tail.compare_exchange_weak(old_tail,new_tail)&&
              old_tail.ptr==current_tail_ptr){
                //qDebug() << "compare_exchange_weak"<< endl;
              }
    ...
    bool push(T&& element){
       ...
       std::unique_ptr<T> new_data = (std::make_unique<T>(std::forward<T>(element)));
    ...
    bool try_pop(T &res){
        std::unique_ptr<T> result = std::move(pop());
        if(result){
...
#endif // LOCK_FREE_QUEUE_PACKAGED_TASK_H
